...
import { LayoutDashboard, Calendar, BarChart3, Users, Receipt, Settings, X, Sun, Moon, LogOut } from "lucide-react"
import { useTheme } from "@/lib/theme-context"
import { fetchApi, clearAuthToken } from "@/lib/api"

const tabs = [
  { href: "/admin", icon: LayoutDashboard, label: "Home" },
  { href: "/admin/daily", icon: Calendar, label: "Daily" },
  { href: "/admin/monthly", icon: BarChart3, label: "Monthly" },
  ...
  return (
    <>
      ...
      <nav
        ...
        style={{ borderTopColor: "var(--color-border)", background: "var(--color-sidebar)" }}
        role="navigation"
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className="relative flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 text-[10px] font-semibold transition-all duration-150"
                style={{ color: isActive ? "var(--color-sidebar-text-active)" : "var(--color-sidebar-text)" }}
              >
                <div
                  className="flex h-8 w-8 items-center justify-center rounded-lg transition-all duration-150"
                  style={isActive ? { background: "var(--color-sidebar-active-bg)" } : undefined}
                >
      ...
      {open && (
        <div className="fixed inset-0 z-[60] md:hidden" onClick={() => setOpen(false)}>
          ...
          <div
            className="absolute bottom-0 left-0 right-0 rounded-t-2xl pb-8 pt-2 safe-bottom"
            style={{ background: "var(--color-sidebar)" }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mx-auto mb-6 h-1 w-10 rounded-full" style={{ background: "var(--color-border)" }} />
